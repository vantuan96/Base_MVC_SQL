...
namespace Kztek.Service.Admin
{
    ...
    public class MenuFunctionService : IMenuFunctionService
    {
            ...
            try
            {
                ...
                foreach (var id in ids)
                {
                    var objDelete = (from m in _MenuFunctionRepository.Table
                                    where m.Breadcrumb.Contains(id) && m.IsDeleted == true
                                    select m).ToList();
                    ...
                    if (objDelete.Any())
                    {
                        foreach (var item in objDelete)
                        {
                            if (!item.Active)
                            {
                                ...
                                _LogService.WriteLog(report, "MenuFunction", item.Id, ActionConfig.Delete, user);
                            }
                        }
        ...
        {
            var query = from n in _MenuFunctionRepository.Table
                        where !n.IsDeleted && n.Active && n.ParentId != "0"
                        select n;
            return query;
        }
        ...
        {
            var query = from n in _MenuFunctionRepository.Table
                        where n.ParentId == "0" && !n.IsDeleted
                        select n;
        ...
        {
            var query = from n in _MenuFunctionRepository.Table
                        where !n.IsDeleted && n.ParentId.Equals(id)
                        orderby n.OrderNumber ascending
                        select n;
            return query;
        }
        ...
        {
            var query = from n in _MenuFunctionRepository.Table
                        where n.ParentId == "0" && !n.IsDeleted && n.Active
                        select n;
            return query;
        }
        ...
        {
            var query = from n in _MenuFunctionRepository.Table
                        where !n.IsDeleted && n.ParentId.Equals(id) && n.Active
                        orderby n.OrderNumber ascending
                        select n;
            return query;
        }
        ...
        public List<MenuFunction> GetAllMenuByPermisstion(string uId, bool isAdmin = false)
        {
            var query = from m in _MenuFunctionRepository.Table
                        ...
                        where ur.UserId == uId && m.Active && !m.IsDeleted /*&& m.MenuType == "1"*/
            ...
            if (isAdmin)
            {
                query = from m in _MenuFunctionRepository.Table
                        join rm in _RoleMenuRepository.Table on m.Id equals rm.MenuId into rMenu
                        from rm in rMenu.DefaultIfEmpty()
                        where m.Active && !m.IsDeleted /*&& m.MenuType == "1"*/
                        orderby m.OrderNumber
                        select m;
            }
            ...
            try
            {
                ...
                if (model != null)
                {
                    ...
                    else if (typ == "update") // trường hợp cập nhập menu
                    {
                        ...
                        else
                        {
                            ...
                            var oldBreadCrumbs = (from m in _MenuFunctionRepository.Table
                                                  where m.Breadcrumb.Contains("/" + model.Id + "/") && m.IsDeleted == false
                                                  select m).ToList();
                    ...
                    else if (typ == "del") // trường hợp xóa menu
                    {
                        //remove bỏ những breadCrumb cũ
                        var oldBreadCrumbs = from m in _MenuFunctionRepository.Table
                                             where m.IsDeleted == false && m.Breadcrumb.Contains("/" + model.ParentId + "/")
                                             select m;
        ...
        public IEnumerable<MenuFunction> GetAllChildActiveByParentId(string id)
        {
            var model = from n in _MenuFunctionRepository.Table
                        where n.ParentId == id && n.Active && !n.IsDeleted
                        select n;
